...
OUTPUT_PATH = "/home/c5mcv07/C5_G7_MCV/Week_2/yolo/src"  # Output inside YOLO folder
...
def extract_polygon_from_rle(rle, height, width):
    # Convert the RLE to a binary mask
    rle_obj = {'counts': rle, 'size': [height, width]}
    binary_mask = maskUtils.decode(rle_obj)

    # Find contours (polygonal mask)
    contours, _ = cv2.findContours(binary_mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    
    # Assuming the first contour corresponds to the object
    if contours:
        contour = contours[0]
        # Flatten the contour points into a single list of x, y points
        polygon = contour.reshape(-1, 2)

        # Normalize the polygon points to YOLO format (scaled between 0 and 1)
        normalized_polygon = polygon / [width, height]  # Normalize to [0, 1] range
    ...
    with open(seq_annot_file, "r") as f:
        for line in f:
            try:
                ...
                polygon = extract_polygon_from_rle(rle, height, width)
                ...
                top_right_x, top_right_y = (x + w)/width, y/height
                bottom_right_x, bottom_right_y = (x + w)/width, (y+h)/height
                bottom_left_x, bottom_left_y = (x)/width, (y+h)/height

                norm_w, norm_h = w / width, h / height
                #annotation = f"{class_id} {top_left_x:.6f} {top_left_y:.6f} {top_right_x:.6f} {top_right_y:.6f} {bottom_right_x} {bottom_right_y} {bottom_left_x} {bottom_left_y}"
                annotation = f"{class_id} "
                annotation += " ".join([f"{p[0]:.6f} {p[1]:.6f}" for p in polygon])
