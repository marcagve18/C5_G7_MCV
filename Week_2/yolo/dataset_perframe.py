...
def convert_kitti_mots_to_yolo(dataset_path, sequence="0000", frame_to_process="000000"):
    ...
    for line in annotations:
        ...
        frame, _, class_id, height, width, rle = parts[:6]
        frame = int(frame)
        ...
        mask = {"counts": rle.strip(), "size": [int(height), int(width)]}
        ...
        x_center = (x + w / 2) / int(width)
        y_center = (y + h / 2) / int(height)
        w = w / int(width)
        h = h / int(height)

        if frame not in ann_dict:
            ann_dict[frame] = []
        ann_dict[frame].append(f"{category_map[class_id]} {top_left_x:.6f} {top_left_y:.6f} {top_right_x:.6f} {top_right_y:.6f} {bottom_right_x} {bottom_right_y} {bottom_left_x} {bottom_left_y}")
